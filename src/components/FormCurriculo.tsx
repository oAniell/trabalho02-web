...
export type CurriculoFormData = Omit<Curriculo, "id" | "createdAt">;
...
function isValidMonthYear(dateStr: string): boolean {
  ...
  return month >= 1 && month <= 12 && year >= 1900 && year <= new Date().getFullYear() + 10;
}
...
const experienciaSchema = yup.object({
  ...
  dataInicio: yup
    ...
    .test("valid-month-year", "Mes ou ano invalido no inicio da experiencia.", (val) => !val || isValidMonthYear(val)),
  dataFim: yup
    .string()
    .trim()
    .required("Informe o fim da experiencia.")
    .matches(/^\d{2}\/\d{4}$/, "Use o formato MM/AAAA no fim da experiencia.")
    .test("valid-month-year", "Mes ou ano invalido no fim da experiencia.", (val) => !val || isValidMonthYear(val))
    .test("after-start", "Data de fim deve ser igual ou posterior ao inicio.", function (val) {
      const start = parseMonthYear(this.parent.dataInicio);
      const end = parseMonthYear(val ?? "");
...
const formacaoSchema = yup.object({
  ...
  dataInicio: yup
    ...
    .test("valid-month-year", "Mes ou ano invalido no inicio da formacao.", (val) => !val || isValidMonthYear(val)),
  dataFim: yup
    .string()
    .trim()
    .required("Informe o fim da formacao.")
    .matches(/^\d{2}\/\d{4}$/, "Use o formato MM/AAAA no fim da formacao.")
    .test("valid-month-year", "Mes ou ano invalido no fim da formacao.", (val) => !val || isValidMonthYear(val))
    .test("after-start", "Data de fim deve ser igual ou posterior ao inicio.", function (val) {
      const start = parseMonthYear(this.parent.dataInicio);
      const end = parseMonthYear(val ?? "");
      if (!start || !end) return true;
...
const schema: yup.ObjectSchema<CurriculoFormData> = yup.object({
  ...
  cpf: yup
    ...
    .test("valid-cpf", "CPF invalido.", (val) => !val || isValidCPF(val)),
  dataNascimento: yup
    .string()
    .required("Informe a data de nascimento.")
    .matches(/^\d{2}\/\d{2}\/\d{4}$/, "Use o formato DD/MM/AAAA na data de nascimento.")
    .test("valid-date", "Data de nascimento invalida.", (val) => !val || isValidDate(val))
    .test("not-future", "Data de nascimento nao pode ser no futuro.", (val) => {
      if (!val || !isValidDate(val)) return true;
      const [d, m, y] = val.split("/").map(Number);
      return new Date(y, m - 1, d) <= new Date();
    })
    .test("min-age", "Idade minima de 14 anos.", (val) => !val || !isValidDate(val) || isAtLeast14(val)),
  ...
  linkedin: yup
    ...
    .trim()
    .optional()
    .test("valid-linkedin", "Informe uma URL valida do LinkedIn (ex: linkedin.com/in/perfil).", (val) => {
      if (!val) return true;
  ...
  github: yup
    ...
    .trim()
    .optional()
    .test("valid-github", "Informe uma URL valida do GitHub (ex: github.com/usuario).", (val) => {
      if (!val) return true;
...
const emptyExperience = { empresa: "", cargo: "", dataInicio: "", dataFim: "", descricao: "" };
const emptyEducation = { instituicao: "", curso: "", grau: "", dataInicio: "", dataFim: "" };
...
function savePhotoPreview(fileName: string, dataUrl: string) {
  ...
  localStorage.setItem(PHOTO_PREVIEWS_KEY, JSON.stringify({ ...previews, [fileName]: dataUrl }));
}
...
export function FormCurriculo({
  defaultValues,
  onSubmit,
  submitLabel = "Salvar curriculo",
  onCancel,
  title = "Novo curriculo",
  subtitle = "Preencha seus dados profissionais para criar um curriculo completo.",
}: FormCurriculoProps) {
  ...
  // Valida no mount para que o botão reflita o estado real desde o início
  // (especialmente importante no modo edição onde os campos já vêm preenchidos)
  useEffect(() => {
    void trigger();
  ...
  const handlePhotoChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      if (typeof reader.result === "string") savePhotoPreview(file.name, reader.result);
    };
  ...
  const handleFormSubmit = async (data: CurriculoFormData) => {
    try {
      await onSubmit(data);
    } catch (err) {
      console.error("Erro ao salvar curriculo:", err);
  ...
  return (
    ...
    >
      <div className="flex flex-col gap-2">
        ...
        <h1 className="text-3xl font-bold text-violet-950">{title}</h1>
        <p className="max-w-2xl text-sm text-violet-700">{subtitle}</p>
      </div>
          ...
          {formacoes.fields.map((field, index) => (
            <div key={field.id} className="grid gap-4 rounded-lg border border-violet-100 p-4 md:grid-cols-2">
              ...
              <Field label="Grau" error={errors.formacoes?.[index]?.grau?.message}>
                <Input
                  {...register(`formacoes.${index}.grau`)}
                  placeholder="Bacharelado, Tecnologo, Pos-graduacao..."
                  aria-invalid={!!errors.formacoes?.[index]?.grau}
                />
              </Field>
              <MaskedField
                control={control}
      ...
      <div className="flex justify-end gap-3">
        {onCancel && (
          <Button type="button" variant="outline" onClick={onCancel} className="h-10 border-violet-200 px-6">
            Cancelar
          </Button>
        )}
        <Button
      ...
      <Controller
        ...
        render={({ field }) => (
          <Input
            ...
            aria-invalid={!!error}
            onChange={(event) => {
              const newRaw = event.target.value.replace(/\D/g, "");
              const prevRaw = String(field.value ?? "").replace(/\D/g, "");
              const digits =
                newRaw.length === prevRaw.length && event.target.value.length < String(field.value ?? "").length
                  ? newRaw.slice(0, -1)
                  : newRaw;
              field.onChange(applyMask(digits, mask));
            }}
          />
        )}
      />
